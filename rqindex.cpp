...
int Deep_Scan(MEMX & MF,int MAX_MISMATCHES, LEN & L,BWT* fwfmi,BWT* revfmi,int Max_Hits)
{
...
//One:
	if ((Opt_Mis==0 ||!Hits) && MAX_MISMATCHES >0)
	{
		In_Mis=1;
...
//Two:
	if ((Opt_Mis==1 ||!Hits) && MAX_MISMATCHES >1)
	{
		In_Mis=2;
...
//Three:
	if ((Opt_Mis==2 ||!Hits) && MAX_MISMATCHES >2)
	{
		In_Mis=3;
...
//Four:
	if ((Opt_Mis==3 ||!Hits) && MAX_MISMATCHES >3)
	{
		In_Mis=4;
...
//Five:
	if ((Opt_Mis==4 || !Hits) && MAX_MISMATCHES >4)
	{
		In_Mis=5;
...
void Search_Small_Gap(SARange & Head, SARange & Tail, unsigned d,PAIR* Pairs,int & Pairs_Index)
{
	...
	if (Pairs_Index==MAX_HITS_TO_STORE) return;
