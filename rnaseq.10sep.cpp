...
int Find_Pairings(int & Pairs_Index,SARange* MF_Pre_Hits,SARange* MF_Suf_Hits,PAIR* &  Pairs, char* Read,int STRINGLENGTH,int & Final_Juncs_Ptr,Junction *Final_Juncs,int & Min_Mismatch,int Mis_In_Anchors);
void Open_Outputs(ofstream & SAM);
inline char* Nullify_String(char* S);
void Print_Hits(READ & Head,Junction *Final_Juncs,FILE* OUT,ofstream & SAM,int Tag_Count);
//}-----------------------------  FUNCTION PRTOTYPES  -------------------------------------------------/*
...
//--------------------- Setup Data Structure for Batman End----------------------------------------
	...
	time_t Maptime;
	{
		...
		while (Read_Tag(Head,Tail,Input_File,Mate_File,File_Info))
		{
			...
			{
				Print_Hits(Head,Final_Juncs,OUT,SAM,Tag_Count);
			}
...
int Find_Single_Junc(char* Read,char* Converted_Read,MEMX & MF_Pre,MEMX & MF_Suf,int STRINGLENGTH,LEN & L,PAIR* & Pairs,Junction *Final_Juncs,int & Err)
{
	...
	if(Last_Mis_Suf >=0)
	{
		Err+=Find_Pairings(Pairs_Index,MF_Pre.Hit_Array,Sub_Opt_Suf_Start,Pairs,Read,STRINGLENGTH,Final_Juncs_Ptr,Final_Juncs,Min_Mismatch,Last_Mis_Suf+M1);
	}
	//------------------------------------ End Higest/Lowest Mismatch Pairing of Suf/Pref -------------------------------------------------
	//------------------------------------ End Higest Mismatch Pairing of Suf/Pref -------------------------------------------------
	if(Last_Mis_Pre >=0 && Last_Mis_Suf>=0)
	{
		Err+=Find_Pairings(Pairs_Index,Sub_Opt_Pre_Start,Sub_Opt_Suf_Start,Pairs,Read,STRINGLENGTH,Final_Juncs_Ptr,Final_Juncs,Min_Mismatch,Last_Mis_Suf+Last_Mis_Pre);
	}
	//------------------------------------ End Higest Mismatch Pairing of Suf/Pref -------------------------------------------------
...
void Print_Hits(READ & Head,Junction *Final_Juncs,FILE* OUT,ofstream & SAM,int Tag_Count)
{
	...
	for(int i=0;Final_Juncs[i].p!=INT_MAX;i++)
	{
		...
		SAM << Head.Description+1 <<"\t"
			...
			<< Final_Juncs[i].p << "\t" 
			<< 60 <<"\t" 
			<< Final_Juncs[i].r << "M" << Final_Juncs[i].q-Final_Juncs[i].p+1<< "I" << READLEN-Final_Juncs[i].r <<"M\t" 
			<< "*\t0\t0\t" 
			<< Head.Tag_Copy << "\t" 
			<< "*\n";
	}
